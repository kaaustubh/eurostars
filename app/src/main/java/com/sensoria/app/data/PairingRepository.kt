...
private val KEY_LEFT_SENSOR_TYPE = stringPreferencesKey("left_sensor_type")
...
private val KEY_RIGHT_SENSOR_TYPE = stringPreferencesKey("right_sensor_type")
...
data class PairedSensor(
    ...
    val rssi: Int? = null,
    val sensorType: SensorType = SensorType.CURRENT
)
...
class PairingRepository(private val context: Context) {
    ...
    val pairingStatusFlow: Flow<PairingStatus> = context.pairingDataStore.data.map { prefs ->
        PairingStatus(
            leftSensor = PairedSensor(
                ...
                sensorType = prefs[KEY_LEFT_SENSOR_TYPE]?.let { 
                    try { SensorType.valueOf(it) } catch (e: IllegalArgumentException) { SensorType.CURRENT }
                } ?: SensorType.CURRENT
            ),
            rightSensor = PairedSensor(
                ...
                sensorType = prefs[KEY_RIGHT_SENSOR_TYPE]?.let { 
                    try { SensorType.valueOf(it) } catch (e: IllegalArgumentException) { SensorType.CURRENT }
                } ?: SensorType.CURRENT
            )
        )
    ...
    suspend fun setLeftSensor(
        ...
        rssi: Int? = null,
        sensorType: SensorType = SensorType.CURRENT
    ) {
        context.pairingDataStore.edit { prefs ->
            ...
            prefs[KEY_LEFT_SENSOR_TYPE] = sensorType.name
        }
    }
    ...
    suspend fun setRightSensor(
        ...
        context.pairingDataStore.edit { prefs ->
            ...
            prefs[KEY_RIGHT_SENSOR_TYPE] = sensorType.name
        }
    }
    ...
    suspend fun clearLeftSensor() {
        context.pairingDataStore.edit { prefs ->
            ...
            prefs.remove(KEY_LEFT_SENSOR_TYPE)
        }
    }
    ...
    suspend fun clearRightSensor() {
        context.pairingDataStore.edit { prefs ->
            ...
            prefs.remove(KEY_RIGHT_SENSOR_TYPE)
        }
    }
